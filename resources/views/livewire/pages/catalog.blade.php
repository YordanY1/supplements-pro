    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
            ...
            <aside class="md:col-span-1 space-y-6">
                ...
                {{-- Categories --}}
                <div class="bg-white/50 p-4 rounded-xl border border-secondary/20">
                        ...
                        @foreach ($categories as $cat)
                            <button wire:click="toggleCategory('{{ $cat['slug'] }}')"
                                class="{{ in_array($cat['slug'], explode(',', $this->category))
                                    ...
                                    : 'bg-secondary/20 text-dark hover:bg-primary/20' }}
                    px-3 py-1.5 rounded-lg text-sm transition font-medium w-full text-left">
                                {{ $cat['name'] }}
                            </button>
                        @endforeach
                    </div>
                ...
                <div class="bg-white/50 p-4 rounded-xl border border-secondary/20">
                    ...
                    <div class="space-y-1.5">
                        <button wire:click="$set('sort','default')"
                            class="{{ $sort === 'default' ? 'bg-primary text-dark' : 'bg-secondary/20 text-dark hover:bg-primary/20' }}
                px-3 py-1.5 rounded-lg text-sm transition font-medium w-full text-left">
                            По подразбиране
                        </button>
                        <button wire:click="$set('sort','price_asc')"
                            class="{{ $sort === 'price_asc' ? 'bg-primary text-dark' : 'bg-secondary/20 text-dark hover:bg-primary/20' }}
                px-3 py-1.5 rounded-lg text-sm transition font-medium w-full text-left">
                            <i class="fas fa-arrow-up text-xs"></i> Цена (ниска-висока)
                        </button>
                        <button wire:click="$set('sort','price_desc')"
                            class="{{ $sort === 'price_desc' ? 'bg-primary text-dark' : 'bg-secondary/20 text-dark hover:bg-primary/20' }}
                px-3 py-1.5 rounded-lg text-sm transition font-medium w-full text-left">
                            <i class="fas fa-arrow-down text-xs"></i> Цена (висока-ниска)
                        </button>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        @foreach ([12, 24] as $count)
                            <button wire:click="$set('perPage',{{ $count }})"
                                class="{{ $perPage == $count ? 'bg-primary text-dark' : 'bg-secondary/20 text-dark hover:bg-primary/20' }}
                    px-3 py-1.5 rounded-lg text-sm transition font-medium">
                                {{ $count }}
                            </button>
                        @endforeach
                    </div>
            ...
            {{-- Products --}}
            <section class="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 auto-rows-fr pb-10">

                @forelse ($products as $product)
                    <div
                        class="bg-secondary/40 p-6 rounded-2xl border border-secondary/40 shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">

                        {{-- Clickable product --}}
                        <a href="{{ route('product.show', $product['slug']) }}"
                            class="block group flex flex-col flex-1">
                            ...
                            <h2
                                class="text-xl font-bold text-primary mb-1 group-hover:text-secondary transition line-clamp-2 min-h-[3.5rem]">
                                {{ $product['title'] }}
                            </h2>

                            <p class="text-sm text-dark/70 mb-1 line-clamp-1">
                                {{ $product['brand_name'] ?? 'Без марка' }}
                            </p>

                            <p class="text-lg font-semibold text-dark mb-3 mt-auto">
                        ...
                        <button wire:click="addToCart('{{ $product['id'] }}')"
                            class="mt-auto w-full px-6 py-2 bg-primary text-dark font-semibold rounded-xl hover:bg-secondary hover:text-white transition cursor-pointer">
                            Добави в количката
                        </button>
                    </div>
                @empty
